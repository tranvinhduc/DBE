...
void SetupKU(int i) // Init for Key r i
{
  ...
  pairing_apply(users[i].eAlpha, bilinearGroup.g, users[i].g_Alpha,
                bilinearGroup.pairing);
...
PartialSecreteKey *GeneratePartialSecreteKey(int i, int j) {
  PartialSecreteKey *partialSecretKey;
  partialSecretKey = (PartialSecreteKey *)malloc(sizeof(PartialSecreteKey));
  ...
  element_mul(partialSecretKey->d0, partialSecretKey->d0,
              users[i].g_Alpha); // Use secret users[i]
...
void freePartialSecreteKey(PartialSecreteKey *partialSecreteKey) {
  element_clear(partialSecreteKey->d0);
  element_clear(partialSecreteKey->d_0);
  for (int j = 0; j < N; ++j) {
    element_clear(partialSecreteKey->d[j]);
  }
  free(partialSecreteKey);
}

// Compute secret key for User j
void Extract(int j) {
  PartialSecreteKey *partialSecreteKey[n];
  for (int i = 0; i < n; ++i) {
    partialSecreteKey[i] = GeneratePartialSecreteKey(i, j);
  }

  element_init_G2(users[j].d.d0, bilinearGroup.pairing);
  element_set1(users[j].d.d0);

  element_init_G2(users[j].d.d_0, bilinearGroup.pairing);
  element_set1(users[j].d.d_0);

  for (int i = 0; i < n; ++i) {
    element_mul(users[j].d.d0, users[j].d.d0, partialSecreteKey[i]->d0);
    element_mul(users[j].d.d_0, users[j].d.d_0, partialSecreteKey[i]->d_0);
  }

  for (int k = 0; k < N; ++k) {
    element_init_G2(users[j].d.d[k], bilinearGroup.pairing);
    element_set1(users[j].d.d[k]);

    for (int i = 0; i < n; ++i) {
      element_mul(users[j].d.d[k], users[j].d.d[k], partialSecreteKey[i]->d[k]);
    }
  }

  // Clear the partial secret keys
  for (int i = 0; i < n; ++i) {
    freePartialSecreteKey(partialSecreteKey[i]);
  }
}
...
void Encrypt(element_t K, Hdr *hdr, RevokedUsers *R) {
  ...
  for (int i = 0; i < R->len; ++i) {
    element_mul(hdr->C2, hdr->C2, param.u[R->r[i]]);
  }
